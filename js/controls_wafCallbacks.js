...
$(document).on('click', '[data-callback-edu-error]', function(e){
    try {
    	Bootstrapper.ensEvent.trigger('edu_error');
    } catch(err) {
...
$(document).on('click', '[data-callback-edu-options-selected]', function(e){
    try {
    	Bootstrapper.ensEvent.trigger('edu_product_options_selected');
    } catch(err) {
    	console.log('Named Event failed');
    }
});
...
$(document).on('click', '[data-callback-edu-product-download]', function(e){
    try {
    	Bootstrapper.ensEvent.trigger('edu_product_download');
    } catch(err) {
    	console.log('Named Event failed');
    }
});
...
$(document).on('click', '[data-callback-edu-trial-cloud]', function(e){
    try {
    	Bootstrapper.ensEvent.trigger('edu_product_cloud_access');
    } catch(err) {
    	console.log('Named Event failed');
    }
});
...
$(document).on('click', '[data-campaign]', function(e){
	...
	switch (which) {
	    ...
	    case "dynamic-external" :
	    	try { analytics.callback.setExternalCampaign("128128"); } catch(err) { }
			break;
	    case "dynamic-internal" :
	    	try { analytics.callback.setInternalCampaign("821821"); } catch(err) { }
			break;
		default:
			break;
